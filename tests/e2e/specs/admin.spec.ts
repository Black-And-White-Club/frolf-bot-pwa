...
await visitAdmin({ arrangeSnapshot, arrangeAuth, wsConnect }, 'player');
...
await visitAdmin({ arrangeSnapshot, arrangeAuth, wsConnect }, 'admin');
...
await expect(page.getByText('#7 is already assigned to another row')).toBeVisible();
...
const entries = wsAssertPublished('leaderboard.batch.tag.assignment.requested.v2');
...
wsEmit('leaderboard.batch.tag.assigned.v2', {
assignment_count: 1,
...
wsEmit('leaderboard.batch.tag.assignment.failed.v2', {
batch_id: payload.batch_id,
guild_id: payload.guild_id,
requesting_user_id: payload.requesting_user_id,
...
await expect(page.getByText('Tag number reserved')).toBeVisible();
});
...
const entries = wsAssertPublished('leaderboard.manual.point.adjustment.v2');
const lastEntry = entries[entries.length - 1];
const payload = lastEntry.payload as {
guild_id: string;
member_id: string;
...
wsEmit('leaderboard.manual.point.adjustment.success.v2', {
guild_id: payload.guild_id,
...
await expect(page.getByText('Points adjusted: +15')).toBeVisible();
});
...
wsEmit('leaderboard.manual.point.adjustment.failed.v2', {
guild_id: payload.guild_id,
reason: 'Only admins can adjust points'
});

await expect(page.getByText('Only admins can adjust points')).toBeVisible();
});
...
const entries = wsAssertPublished('round.scorecard.admin.upload.requested.v2');
const lastEntry = entries[entries.length - 1];
const payload = lastEntry.payload as {
guild_id: string;
round_id: string;
...
const checkEntries = wsAssertPublished('round.admin.backfill.check.v1');
...
const submitEntries = wsAssertPublished('round.admin.backfill.requested.v1');
