...
if (shouldStubInvites) {
await page.waitForResponse(`**/api/clubs/${subjectId}/invites`);
}
}
...
const account = new AccountPage(page);
await visitAccount({ page, arrangeSnapshot, arrangeAuth, wsConnect }, {
role: 'admin',
...
await account.discordRow().getByRole('button', { name: 'Disconnect' }).click();
await page.waitForResponse('**/api/auth/discord/unlink');
...
await account.createInviteButton().click();
...
await account.clickRevoke('REVOKE001');
await page.waitForResponse(`**/api/clubs/${subjectId}/invites/REVOKE001`);
