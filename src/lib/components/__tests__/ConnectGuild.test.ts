...
const mockList = async () => [{ id: 'g1', name: 'G1', isAdmin: true }, { id: 'g2', name: 'G2', isAdmin: false }]
const mockLink = async (id: string) => ({ success: id === 'g1', error: id === 'g2' ? 'not_admin' : undefined })

test('renders guilds and handles link', async () => {
  const { getByText } = render(ConnectGuild, { props: { listGuilds: mockList, linkGuild: mockLink } })
