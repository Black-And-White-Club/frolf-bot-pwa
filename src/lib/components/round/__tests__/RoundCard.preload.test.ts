...
test('RoundCard preload branch loads components when observe triggers', async () => {
  ...
  const ParticipantStub = { default: { render: () => '<div data-testid="participant-stub">P</div>' } }
  const DetailsStub = { default: { render: () => '<div data-testid="details-stub">D</div>' } }

  participantLoader.__TEST_HOOKS.injectedImport = async () => ParticipantStub as unknown as Promise<unknown>
  detailsLoader.__TEST_HOOKS.injectedImport = async () => DetailsStub as unknown as Promise<unknown>
...
test('loader injection can simulate failures and RoundCard falls back gracefully', async () => {
  ...
  participantLoader.__TEST_HOOKS.injectedImport = async () => { throw new Error('fail import') }
  // Details succeed
  detailsLoader.__TEST_HOOKS.injectedImport = async () => ({ default: {} })

  const round = makeRound()
  const { getByTestId, queryByText } = render(RoundCard, { props: { round, dataTestId: 'rc-pre-2' } })
