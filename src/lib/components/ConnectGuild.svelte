...
<style>
.message { margin-top: .5rem; color: green }
.container { background: var(--card-bg, #fff); padding: .5rem; border-radius: .5rem }
.guild-list { list-style: none; margin: 0; padding: 0 }
.guild-item { display:flex; justify-content:space-between; align-items:center; padding:.4rem 0; border-bottom: 1px solid #eee }
.guild-info { display:flex; flex-direction:column }
.guild-meta { font-size: .8rem; color: #666 }
.guild-actions { margin-left: 1rem }
.btn-link { background: #0b84ff; color: white; border: none; padding: .3rem .6rem; border-radius: .25rem }
.btn-disabled { background: #ddd; color: #666; border: none; padding: .3rem .6rem; border-radius: .25rem }
