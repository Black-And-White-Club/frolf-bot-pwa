<script lang="ts">
  ...
  function makeEnvelope(e: EnvelopeLike): EnvelopeLike { return e }
  ...
  function sendPatch() {
    const patch: EnvelopeLike = makeEnvelope({ type: 'patch', schema: 'leaderboard.patch.v1', version: 2, ts: new Date().toISOString(), op: 'upsert_player', payload: { userId: 'u2', name: 'Dana', score: 8 } })
