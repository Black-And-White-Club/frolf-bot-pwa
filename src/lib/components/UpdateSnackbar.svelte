...
<script lang="ts">
  ...
  function handleShow(ev: Event) {
    ...
    try { prevActive = document.activeElement } catch { prevActive = null }
    visible.set(true)
  ...
  function refresh() {
    ...
    if (worker && typeof worker.postMessage === 'function') worker.postMessage({ type: 'SKIP_WAITING' })
  ...
  function dismiss() {
    ...
    try { if (prevActive && (prevActive as HTMLElement).focus) (prevActive as HTMLElement).focus() } catch {}
  }
</script>

{#if $visible}
  <div class="fixed bottom-4 left-4 right-4 z-50 flex items-center justify-between p-3 bg-[var(--guild-surface)] border rounded-lg shadow-lg" role="status" aria-live="polite">
    <div class="text-sm">A new version is available.</div>
    <div class="flex items-center space-x-2">
      <button bind:this={refreshBtn} class="px-3 py-1 bg-[var(--guild-primary)] text-white rounded" on:click={refresh} on:keydown={(e) => e.key === 'Enter' && refresh()}>Refresh</button>
      <button class="px-3 py-1 border rounded" on:click={dismiss}>Dismiss</button>
    </div>
  </div>
{/if}
