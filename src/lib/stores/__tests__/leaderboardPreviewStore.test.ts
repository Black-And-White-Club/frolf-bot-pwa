...
describe('LeaderboardPreviewStore', () => {
  it('applies snapshot and ignores older patches', async () => {
    ...
    const oldPatch: Envelope = {
      type: 'patch', schema: 'leaderboard.patch.v1', version: 1, op: 'upsert_player', ts: new Date().toISOString(), payload: { userId: 'u2', name: 'Dana', score: 8 }
    }
  ...
  expect(state!.snapshot!.topPlayers.length).toBe(1)
    ...
    // send v2 patch
    const patch2: Envelope = { type: 'patch', schema: 'leaderboard.patch.v1', version: 2, op: 'upsert_player', ts: new Date().toISOString(), payload: { userId: 'u2', name: 'Dana', score: 8 } }
