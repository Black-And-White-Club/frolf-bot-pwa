	...
	<div class="mx-auto max-w-2xl space-y-8">
		...
		<section>
			...
			<div class="space-y-3">
				...
				>
					...
					{#if isDiscordLinked}
						<div class="flex items-center gap-3">
							<span class="text-guild-primary font-['Space_Grotesk'] text-sm font-medium"
								>Connected</span
							>
							<button
								onclick={() => unlinkProvider('discord')}
								disabled={unlinkingProvider === 'discord'}
								class="rounded-lg border border-red-500/30 px-3 py-1.5 font-['Space_Grotesk'] text-xs text-red-400 transition-colors hover:bg-red-500/10 disabled:opacity-50"
							>
					...
					{:else}
						<a
							...
							class="bg-liquid-skobeloff rounded-lg px-4 py-2 font-['Space_Grotesk'] text-sm font-medium text-white transition-all hover:brightness-110"
						>
							Connect Discord
						</a>
					{/if}
		...
		{#if canManageInvites}
			<section>
				...
				<!-- Create form -->
				<div class="mb-6 rounded-xl border border-[#007474]/20 bg-[var(--guild-surface)] px-5 py-5">
					<h3
						class="mb-4 font-['Space_Grotesk'] text-sm font-semibold tracking-wide text-[var(--guild-text-secondary)] uppercase"
					>
						Create Invite
					</h3>
					<div class="flex flex-wrap items-end gap-3">
						<div class="flex flex-col gap-1">
							...
							<select
								...
								class="rounded-lg border border-[#007474]/30 bg-[var(--guild-surface-elevated)] px-3 py-2 font-['Space_Grotesk'] text-sm text-[var(--guild-text)] focus:ring-1 focus:ring-[#007474] focus:outline-none"
							>
						...
						<div class="flex flex-col gap-1">
							...
							<input
								...
								class="w-28 rounded-lg border border-[#007474]/30 bg-[var(--guild-surface-elevated)] px-3 py-2 font-['Space_Grotesk'] text-sm text-[var(--guild-text)] placeholder:text-[var(--guild-text-muted)] focus:ring-1 focus:ring-[#007474] focus:outline-none"
							/>
						</div>
						<div class="flex flex-col gap-1">
							<label
				...
				{:else}
					<div class="space-y-3">
						{#each invites as invite (invite.code)}
							...
							>
								<!-- Code -->
								<code class="min-w-0 flex-1 font-mono text-sm break-all text-[var(--guild-text)]">
									{invite.code}
								</code>

								<!-- Role badge -->
								<span
									class="rounded-full px-2 py-0.5 font-['Space_Grotesk'] text-xs font-medium {roleBadgeClass(
										invite.role
									)}"
								>
									{invite.role}
								...
								<!-- Uses -->
								<span
									class="font-['Space_Grotesk'] text-xs whitespace-nowrap text-[var(--guild-text-secondary)]"
								>
								...
								<div class="flex gap-2">
									<button
										...
										class="rounded-lg border border-[#007474]/30 px-3 py-1.5 font-['Space_Grotesk'] text-xs text-[var(--guild-text-secondary)] transition-colors hover:text-[var(--guild-text)]"
									>
										{copyFeedback[invite.code] ? 'Copied!' : 'Copy Link'}
									</button>
									<button
										onclick={() => revokeInvite(invite.code)}
										class="rounded-lg border border-red-500/30 px-3 py-1.5 font-['Space_Grotesk'] text-xs text-red-400 transition-colors hover:bg-red-500/10"
									>
										Revoke
									</button>
								</div>
