...
export const POST: RequestHandler = async ({ fetch, request, url }) => {
	...
	const body = (await request.json().catch(() => null)) as WalletAdjustmentRequest | null;
	if (
		!body ||
	...
	const res = await fetch(`${serverConfig.backendUrl}/api/betting/admin/wallet-adjustments`, {
		method: 'POST',
		headers: {
