...
/**
 * Proxy the Google OAuth link initiation to the backend.
 * Requires a valid refresh_token cookie (backend enforces this).
 * Sets link_mode + oauth_state cookies and redirects to Google.
 *
 * GET /api/auth/google/link
 */
export const GET: RequestHandler = async ({ fetch, request }) => {
	const res = await fetch(`${serverConfig.backendUrl}/api/auth/google/link`, {
		redirect: 'manual',
