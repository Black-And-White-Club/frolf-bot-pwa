 ...
 * @property {{service: string, module: string}} producer Legacy singular producer retained for backwards compatibility.
 * @property {{service: string, module: string}[]} [producers] Authoritative producer list when present.
...
export function validatePayloadWithSchema(schema, value) {
	...
	validateSchema(schema, value, '$', errors, warnings);
	for (const warning of warnings) {
		console.warn(`[ws-contract-testkit] ${warning}`);
	}
	return errors;
}
 ...
 */
function validateSchema(schema, value, pathLabel, errors, warnings) {
	...
	switch (schemaType) {
		case 'object': {
			const bypassWarning = getGeneratedObjectSchemaBypassWarning(schema, pathLabel);
			if (typeof value === 'string' && bypassWarning) {
				warnings.add(bypassWarning);
				return;
			}
			validateObjectSchema(schema, value, pathLabel, errors, warnings);
			break;
		}
		case 'array': {
			const bypassWarning = getGeneratedIdArraySchemaBypassWarning(schema, pathLabel);
			if (typeof value === 'string' && bypassWarning) {
				warnings.add(bypassWarning);
				return;
			}
			validateArraySchema(schema, value, pathLabel, errors, warnings);
			break;
		}
		case 'string':
 ...
 * @param {Set<string>} warnings
 */
function validateObjectSchema(schema, value, pathLabel, errors, warnings) {
	if (!isObject(value)) {
	...
	for (const [key, childSchema] of Object.entries(properties)) {
		if (key in value) {
			validateSchema(childSchema, value[key], `${pathLabel}.${key}`, errors, warnings);
		}
	}
 ...
 * @param {Set<string>} warnings
 */
function validateArraySchema(schema, value, pathLabel, errors, warnings) {
	...
	value.forEach((entry, index) => {
		validateSchema(itemSchema, entry, `${pathLabel}[${index}]`, errors, warnings);
	});
}
 ...
 * empty object schema. Accept string timestamps for these fields, but warn so
 * the caller can spot schema drift rather than silently bypassing validation.
 *
 * @param {Record<string, unknown>} schema
 * @param {string} pathLabel
 * @returns {string | null}
 */
function getGeneratedObjectSchemaBypassWarning(schema, pathLabel) {
	...
	if (!normalizedPath.includes('time') && !normalizedPath.includes('date')) {
		return null;
	}

	return `Skipping strict object validation for ${pathLabel}: generated empty-object schema is being treated as a string time/date field.`;
}

/**
 * Contract schema generation from Go UUID types may represent IDs as integer
 * byte arrays. Accept string IDs for known ID-like fields, but warn so the
 * caller can spot schema drift rather than silently bypassing validation.
 *
 * @param {Record<string, unknown>} schema
 * @param {string} pathLabel
 * @returns {string | null}
 */
function getGeneratedIdArraySchemaBypassWarning(schema, pathLabel) {
	...
	return `Skipping strict array validation for ${pathLabel}: generated integer-array schema is being treated as a string ID field.`;
}
